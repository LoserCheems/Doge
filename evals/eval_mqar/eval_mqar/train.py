...
from eval_mqar.data.utils import prepare_data
from eval_mqar.config import TrainConfig
from eval_mqar.model import LanguageModel
from eval_mqar.logger import WandbLogger
from eval_mqar.utils import set_determinism
...
class Trainer:
    ...
    def train_epoch(self, epoch_idx: int, gradient_accumulation_steps: int = 1):
        self.model.train()
        iterator = tqdm(
        ...
        for batch_idx, (inputs, targets, slices) in enumerate(iterator):
            ...
            # Save original loss for logging
            ...
            if (batch_idx + 1) % gradient_accumulation_steps == 0 or (batch_idx + 1) == len(self.train_dataloader):
                self.optimizer.step()
                self.optimizer.zero_grad()

            # logging and printing
            iterator.set_postfix({"loss": original_loss})
            self.logger.log(
                {
                    "train/loss": original_loss,
    ...
    def test(self, epoch_idx: int):
        ...
        with torch.no_grad(), tqdm(
            ...
            self.logger.log({"epoch": epoch_idx + 1, **metrics})
        return metrics
    ...
    def fit(self):
        ...
        for epoch_idx in range(self.max_epochs):
            self.train_epoch(epoch_idx, gradient_accumulation_steps=self.gradient_accumulation_steps)
